...
(define (sudoku-solver sudoku-problem)
  ...
  (define (CheckLegality StateIndex)
     ...
     (define (box-starting-index position-value)
       (cond
         ((or (and (= (floor (/ position-value 3)) 0) (> (remainder position-value 3) 0))
              (and (= (floor (/ position-value 3)) 0) (= (remainder position-value 3) 0))
     ...
     (display "box starting row position: ")
     (display box-starting-row-position) (newline)

     (display "box starting column position: ")
     (display box-starting-column-position) (newline)
  ...
  ;(define (SolveSudokuProblem)    
   ...
   (define InitialState (list (list 0 3) 5 ))
  ...
  (display "updated sudoku solution: ")
   (display sudoku-solution) (newline)


  (display "next empty position1")
  (display (find-next-empty-position 0 3)) (newline)

  (display "nextMove after initialState")
  (set! Path (append Path (list (nextMove (car InitialState) 0))))
  (display "Updated Path: ")
  (display Path) (newline)

  (display "sudoku-solution after successful next move:")
  (display sudoku-solution) (newline)

  (display "nextMove-2")
  (set! Path (append Path (list (nextMove (list-ref (list-ref Path (- (length Path) 1)) 0) 0))))
  (display "Updated Path 2: ")
  (display Path) (newline)
  

  (display "sudoku-solution after successful next move 2:")
  (display sudoku-solution) (newline)

  (display "nextMove-3")
  (set! Path (append Path (list (nextMove (list-ref (list-ref Path (- (length Path) 1)) 0) 0))))
  (display "Updated Path 3: ")
  (display Path) (newline)
  

  (display "sudoku-solution after successful next move 3:")
  (display sudoku-solution) (newline)

  
  (display "Check final state for: ")
  (display "Last State Index of Path: ")
    (newline)
    (display Path) (newline)
    (display (checkFinalState (list-ref (list-ref Path (- (length Path) 1)) 0)))
