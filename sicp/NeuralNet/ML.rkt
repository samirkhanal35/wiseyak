...
(define (Fit model features target epochs output-categories learning-rate)
  ...
  (display no-of-output-neurons)(newline)
  ...
  (define (propagate layered-weights)
    (cond
      ...
      (else (display "\n forward propagation completed\n")
            (display "\nFinal Prev-layer-neurons:")
            (display prev-layer-neurons)(newline)
            
            (list all-neurons (softmax-activation prev-layer-neurons))            
            )
      )
    )
  (propagate weights)
...
(define (chain-rule numerator-function denominator-function error)
  ...
  (define numerator-elements (flatmap numerator-function))
...
(define (gradient Error-function denominator-function layer model neurons error)
  ...
  (chain-rule numerator-function error)
  )
...
(define (backward-propagation outputs target-outputs all-outputs neurons weights model-length learning-rate)
  ...
  (display "last layer outputs:")
  (display outputs)(newline)

  (display "target outputs:")
  (display target-outputs) (newline)

  (define no-of-outputs (length outputs))
  
  
  (display "\n Number of outputs")
  (display no-of-outputs) (newline)

  (display "\n Number of data for all neurons:")
  (display (length all-outputs))(newline)
