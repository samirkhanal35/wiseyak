...
(define (sudoku-solver sudoku-problem)
  ...
  (define (update-all-possiblestates-arc-consistancy temp-unique-elements temp-states)
    (display "\nInside update all possible states arc consistancy")(newline)
    (display "passed temp states: ")
    (display temp-states) (newline)
    )

  (define (arc-operation unique-list list-element all-elements)
    ...
    (define (check-operation new-state)
      ...
      (set! temp-unique-elements (get-unique-elements unique-elements curr-element))
      ...
      (cond
        ((eq? (length temp-unique-elements) (length temp-states))
         ...
         (update-all-possiblestates-arc-consistancy temp-unique-elements temp-states)
         '#t
         )
        (else
         '#f)
  ...
  (define (propagation-process unique-list list-element)
    ...
    (cond
      ((arc-operation unique-list list-element row-elements) '#t)
      ((arc-operation unique-list list-element column-elements) '#t)
      ((arc-operation unique-list list-element box-elements) '#t)
      (else
       '#f)
      )
    )
  ...
  (define (arcConsistency)
    ...
    (define (propagation)
      ...
      (define list-element (list (list 2 4) 0 1 0))
      (display "\nList element: ")
      (display list-element) (newline)
